...
<div>
    <div x-show="sidebarOpen"
        ...
        class="fixed inset-0 bg-slate-950/60 backdrop-blur-md z-[1008] lg:hidden">
    </div>

    <aside
        :class="sidebarOpen ? 'translate-x-0 w-72' : '-translate-x-full lg:translate-x-0 lg:w-24'"
        class="fixed lg:relative inset-y-0 left-0 z-[1010] glass-card transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1) flex flex-col h-screen border-r border-white/5 shadow-2xl overflow-hidden shrink-0">

        <div class="p-8 flex items-center" :class="sidebarOpen ? 'justify-between' : 'justify-center'">
            <div x-show="sidebarOpen" x-transition.opacity.duration.500ms class="font-black italic tracking-tighter text-blue-500 text-xl uppercase whitespace-nowrap">
        ...
        <div class="p-4 border-t border-white/5 space-y-2 mb-4">
            ...
            <button wire:click="logout" class="w-full flex items-center p-4 rounded-xl transition-all text-red-500/40 hover:text-red-500 hover:bg-red-500/5 group">
                ...
                <span x-show="sidebarOpen" x-transition.opacity.duration.500ms class="ml-4 font-black tracking-widest uppercase text-[10px]">Keluar Sistem</span>
            </button>
        </div>
    </aside>
</div>
