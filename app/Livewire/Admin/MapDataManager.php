...
class MapDataManager extends Component
{
    ...
    public $search = '', $filterCategory = '', $filterType = '';
    ...
    {
        $map = MapData::findOrFail($id);
        Activity::create(['user_name' => auth()->user()->name, 'action' => 'Menghapus', 'subject' => $map->name, 'type' => 'Data Spasial']);
        if ($map->geojson_path) Storage::disk('public')->delete($map->geojson_path);
        if ($map->icon_path) Storage::disk('public')->delete($map->icon_path);
        $map->delete();
        $this->dispatch('notify', message: 'Data dihapus', type: 'error');
    }
